...
router.get('/', asyncHandler(async (req, res) => {
  res.json(await chatsRepo.list(req.clientId, { since: req.query.since }));
}));

router.get('/:id', asyncHandler(async (req, res) => {
  uuid.parse(req.params.id);
  const chat = await chatsRepo.findById(req.clientId, req.params.id);
...
router.post('/', asyncHandler(async (req, res) => {
  ...
  const chat = await chatsRepo.create(req.clientId, data);
...
router.patch('/:id', asyncHandler(async (req, res) => {
  ...
  const chat = await chatsRepo.update(req.clientId, req.params.id, data);
  if (!chat) return res.status(404).json({ error: 'Not found' });
  res.json(chat);
}));
...
router.delete('/:id', asyncHandler(async (req, res) => {
  uuid.parse(req.params.id);
  const deleted = await chatsRepo.remove(req.clientId, req.params.id);
...
router.get('/:id/messages', asyncHandler(async (req, res) => {
  ...
  const chat = await chatsRepo.findById(req.clientId, req.params.id);
  if (!chat) return res.status(404).json({ error: 'Not found' });
  res.json(await chatsRepo.listMessages(req.clientId, req.params.id, { since: req.query.since }));
}));
...
router.post('/:id/messages', asyncHandler(async (req, res) => {
  ...
  try {
    const message = await messagingService.sendChatMessage(req.clientId, chat, data);
...
router.post('/:id/messages/:messageId/retry', asyncHandler(async (req, res) => {
  ...
  const chat = await chatsRepo.findById(req.clientId, req.params.id);
  if (!chat) return res.status(404).json({ error: 'Not found' });
  const message = await chatsRepo.findMessageById(req.clientId, req.params.id, req.params.messageId);
  if (!message) return res.status(404).json({ error: 'Message not found' });

  try {
    const updated = await messagingService.retryMessage(req.clientId, chat, message);
    res.json(updated);
  } catch (err) {
    if (err instanceof messagingService.MessagingError) {
      const status = err.code === 'send_failed' ? 502 : 409;
