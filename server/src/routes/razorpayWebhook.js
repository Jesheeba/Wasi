...
router.post('/', asyncHandler(async (req, res) => {
  ...
  if ((event === 'payment.captured' || event === 'order.paid') && orderId) {
    ...
    const subscription = await subscriptionsRepo.updateByProviderRef(orderId, { status: 'active', renews_at: renewsAt });
    await invoicesRepo.markStatusByOrderId(orderId, 'paid', payment?.id);
    if (subscription) {
      const client = await clientsRepo.findById(subscription.client_id);
      if (client && client.status === 'pending_setup') {
        await clientsRepo.update(subscription.client_id, { status: 'payment_confirmed' });
      }
    ...
    const walletTx = await walletRepo.markStatusByOrderId(orderId, 'completed');
    if (walletTx) {
    ...
    const subscription = await subscriptionsRepo.updateByProviderRef(orderId, { status: 'payment_failed' });
    await invoicesRepo.markStatusByOrderId(orderId, 'failed', payment?.id);
    if (subscription) {
      await auditLogRepo.record({ actor_type: 'razorpay_webhook', actor_id: subscription.client_id, action: event, target: orderId });
    }
    await walletRepo.markStatusByOrderId(orderId, 'failed');
  } else if (event === 'refund.processed' && refund?.payment_id) {
    await invoicesRepo.markStatusByPaymentId(refund.payment_id, 'refunded');
    ...
    if (linkId) {
      await paymentLinksRepo.markStatusByRazorpayId(linkId, 'paid');
